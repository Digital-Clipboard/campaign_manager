...
/**
 * Create Round 3 Contact List in MailJet
 ...
 * 5. Create new list "campaign_batch_003"
 * 6. Add all remaining contacts to the new list
 */
...
async function createRound3List() {
  console.log('🚀 Creating Round 3 Contact List (campaign_batch_003)');
  ...
  try {
    ...
    if (allContacts.length < 2001) {
      ...
      console.error(`   Cannot create Round 3 list.`);
      return;
    }
    ...
    const round3Contacts = allContacts.slice(2000); // No upper limit - take all remaining

    console.log(`   ✅ Extracted ${round3Contacts.length} contacts for Round 3`);
    console.log(`   First Contact ID: ${round3Contacts[0].ContactID}`);
    console.log(`   Last Contact ID: ${round3Contacts[round3Contacts.length - 1].ContactID}\n`);

    // Step 4: Create new list
    console.log('Step 4: Creating new contact list...');
    const newList = await createContactList('campaign_batch_003');

    // Step 5: Add contacts to new list
    console.log('Step 5: Adding contacts to new list...');
    await addContactsToList(newList.ID, round3Contacts);

    // Step 6: Summary
    console.log('\n✅ ROUND 3 LIST CREATED SUCCESSFULLY!');
    console.log('='.repeat(80));
    console.log(`📋 List Name: campaign_batch_003`);
    console.log(`🆔 List ID: ${newList.ID}`);
    console.log(`👥 Total Contacts: ${round3Contacts.length}`);
    console.log(`📊 Contact Range: Users 2,001 to ${allContacts.length} (by registration order)`);
    console.log(`\n📝 NEXT STEPS:`);
    console.log(`1. Wait 1-2 minutes for MailJet to process the upload`);
    console.log(`2. Verify list count in MailJet dashboard`);
    console.log(`3. Create Round 3 campaign in MailJet with List ID: ${newList.ID}`);
    console.log(`4. Use same template and subject as Round 1 & 2`);
    console.log(`5. Schedule for October 2, 2025 at 9:15 AM UTC`);
    console.log(`\n🎉 This completes the entire user base (${allContacts.length} total users)!`);

    return {
      listId: newList.ID,
      listName: newList.Name,
      contactCount: round3Contacts.length,
      totalUsers: allContacts.length
    };
...
module.exports = { createRound3List, getAllContactsFromList, createContactList, addContactsToList };
